...
def test_create_user_new_email(
    ...
    with (
        ...
        created_user = r.json()
        user = services.user.get_user_by_email(session=db, email=username)
        assert user
...
def test_get_existing_user(
    ...
    user = services.user.create_user(session=db, user_create=user_in)
    user_id = user.id
    r = client.get(
    ...
    assert 200 <= r.status_code < 300
    api_user = r.json()
    existing_user = services.user.get_user_by_email(session=db, email=username)
...
def test_create_user_existing_username(
    ...
    password = random_lower_string()
    user_in = UserCreate(email=username, password=password)
    services.user.create_user(session=db, user_create=user_in)
    data = {"email": username, "password": password}
    r = client.post(
        f"{settings.API_V1_STR}/users/",
        headers=superuser_token_headers,
...
def test_retrieve_users(
    ...
    services.user.create_user(session=db, user_create=user_in2)
...
def test_update_user_email_exists(
    ...
    password2 = random_lower_string()
    user_in2 = UserCreate(email=username2, password=password2)
    user2 = services.user.create_user(session=db, user_create=user_in2)
...
def test_delete_user_current_super_user_error(
    ...
    super_user = services.user.get_user_by_email(
